<!doctype html>
<html>
<head>
<meta charset="utf-8">
<link href="ProAlterationStyle.css" rel="stylesheet">
<title>Pro Alterations & Suits</title>
</head>

<body>
	<div>
		<a href="home.php"><img id="logo" src="logo.png" alt="logo"></a>
	</div>
	<div class="navbar">
		<ul>
			<li class="nav" id="firstnav"><a href="home.php">Home</a></li>
			<li class="nav"><a href="about.php">About</a></li>
			<li class="nav"><a href="product.php">Product</a></li>
			<li class="nav" id="lastnav"><a href="service.php">Service</a></li>
		</ul>
	</div>
	<div class="about">
		<h1 class="head">Pro Alterations & Suits is an expert clothing alteration and sewing business with 20 years of experience</h1>
		<h3 class="head">Alteration Services</h3>
		<p class="body">Pro Alterations & Suits do alterations on all type of clothing from formal to casual, wedding gowns, suits, texudos, pants, jeans, shirts, skirts, and more</p>
		<p class="body"> We can mend items, patch tears or holes, and replace zippers. We can hem, take in, and do other alteraions to provide a custom fit</p>
	</div>
	<div>
		<footer>
			<p>All rights reserved to Pro Alteration & Suits</p>
				<a class="socialMedia" href="http://www.instagram.com"><img src="Pictures/instagram.png" alt="Instagram" style="height: 3rem; width: 3rem;"></a>
				<a class="socialMedia" href="http://www.twitter.com"><img src="Pictures/twitter.png" alt="Twitter" style="height: 3rem; width: 3rem;"></a>
				<a class="socialMedia" href="http://www.facebook.com"><img src="Pictures/facebook.png" alt="Facebook" style="height: 3rem; width: 3rem;"></a>
		</footer>
	</div>
</body>
</html>